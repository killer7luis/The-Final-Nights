...
/obj/vehicle/sealed/car/clowncar/after_add_occupant(mob/M, control_flags)
	...
	if(return_controllers_with_flag(VEHICLE_CONTROL_KIDNAPPED).len >= 30)
		for(var/mob/car_man as anything in return_drivers())
			car_man.client.give_award(/datum/award/achievement/misc/round_and_full, car_man)
...
/obj/vehicle/sealed/car/clowncar/proc/roll_the_dice(mob/user)
	if(TIMER_COOLDOWN_CHECK(src, "clown_car_randomness"))
		to_chat(user, span_notice("The button panel is currently recharging."))
		return
	TIMER_COOLDOWN_START(src, "clown_car_randomness", dice_cooldown_time)
	switch(rand(1,6))
		if(1)
...
#undef CLOWN_CANNON_INACTIVE
#undef CLOWN_CANNON_BUSY
#undef CLOWN_CANNON_READY
